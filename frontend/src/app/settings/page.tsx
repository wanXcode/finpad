  ...
  const handleAddAccount = async () => {
    try {
      await api("/api/accounts", {
        method: "POST",
        body: { name: accName, platform: accPlatform, account_type: accType, balance: accBalance },
      });
          ...
          <TabsContent value="profile" className="space-y-4">
            ...
            <Card>
              ...
              <CardContent className="space-y-3">
                ...
                <Button onClick={handleChangePw} disabled={!oldPw || !newPw}>确认修改</Button>
              </CardContent>
            </Card>
          </TabsContent>
          ...
          <TabsContent value="accounts" className="space-y-4">
            <Card>
              <CardHeader>
                ...
                <CardDescription>管理你的支付宝、微信、银行卡等账户及余额</CardDescription>
              </CardHeader>
              <CardContent>
                {accounts.length === 0 ? (
                    ...
                    {accounts.map((a) => (
                      <div key={a.id} className="flex items-center justify-between py-2 border-b last:border-0">
                        <div>
                          ...
                          <Badge variant="outline" className="ml-2">{a.platform}</Badge>
                          <Badge variant="secondary" className="ml-1">{a.account_type}</Badge>
                        </div>
                        <div className="flex items-center gap-3">
                          <span className={`font-medium ${a.balance >= 0 ? "text-green-500" : "text-red-500"}`}>
                            ¥{formatMoney(a.balance)}
                          </span>
                          <Button variant="ghost" size="sm" className="text-destructive" onClick={() => handleDeleteAccount(a.id)}>删除</Button>
                        </div>
                      </div>
                    ))}
                  </div>
                ...
                <div className="flex gap-3 items-end flex-wrap">
                  <div className="space-y-1">
                    ...
                    <Input value={accName} onChange={(e) => setAccName(e.target.value)} placeholder="储蓄卡(尾号)" className="w-36" />
                  </div>
                  <div className="space-y-1">
                  ...
                  <div className="space-y-1">
                    ...
                    <Input type="number" value={accBalance} onChange={(e) => setAccBalance(Number(e.target.value))} className="w-28" />
                  </div>
                  <Button size="sm" onClick={handleAddAccount} disabled={!accName}>添加</Button>
                </div>
              </CardContent>
            </Card>
          </TabsContent>
          ...
          <TabsContent value="categories" className="space-y-4">
            <Card>
              <CardHeader>
                ...
                <CardDescription>
                  将各平台原始分类映射到统一分类。新平台分类首次出现时默认归为"其他"，可在此处修改。
                </CardDescription>
              </CardHeader>
              <CardContent>
                {mappings.length === 0 ? (
                  <p className="text-muted-foreground text-sm">暂无映射规则（系统将在导入数据时自动生成）</p>
                ) : (
                  <div className="space-y-2">
                    {mappings.map((m) => (
